...
public class ProcessStationAnimation
     ...
     * @param pos             middle point of animation object
     * @param form            form of animation object
     * @param listId          unused. For printing a line between waitinglist and this station use the actual version.
     * @param showInAnimation switch animation on or off
     */
    @Deprecated
    public ProcessStationAnimation(ModelAnimation owner, String name,
    ...
    public boolean insert(java.util.List<Proc> procEntities, java.util.List<Res> resEntities) {
        ...
        for (int i = 0; i < procEntities.size(); i++) {
            procEntityIds[i] = procEntities.get(i).getName();
        }
        String[] resEntityIds = new String[resEntities.size()];
        for (int i = 0; i < resEntities.size(); i++) {
            resEntityIds[i] = resEntities.get(i).getName();
        }
    ...
    /**
     * Remove the process entry which contains the entity e. The entity e can be a process or a resource entity. All
     * entities of the removed entry are removed also.
     *
     * @param e
