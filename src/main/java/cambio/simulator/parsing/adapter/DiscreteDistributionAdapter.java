...
/**
 * TODO:!!!!!! Adapter for parsing distributions from JSON into a {@link desmoj.core.dist.DiscreteDist}.
 *
 * <p>
...
public class DiscreteDistributionAdapter extends MiSimModelReferencingTypeAdapter<NumericalDist<Integer>> {
    ...
    public NumericalDist<Integer> read(JsonReader in) throws IOException {
        ...
        if (peeked == JsonToken.STRING) {
            ...
            return new DiscreteDistConstant<>(model, "Constant", constant, false, false);

        } else if (peeked == JsonToken.NUMBER) {
            return new DiscreteDistConstant<>(model, "Constant", in.nextInt(), false, false);
        }
        return null;
    }
}
