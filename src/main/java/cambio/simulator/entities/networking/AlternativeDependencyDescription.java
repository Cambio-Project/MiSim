...
/**
 * An intermediate {@link DependencyDescription} that picks one child dependency based on a
 * probability for execution.
 * 
...
public class AlternativeDependencyDescription extends IntermediateDependencyDescription {
    ...
    /**
     * Creates a new alternative dependency. In order to work properly, the dependency expects at
     * least one child dependency ({@link #setDependencies(DependencyDescription[])}) with
     * {@link AbstractDependencyDescription#getAlternativeProbability()}.
     * 
     * @param model the MiSim model
     * @param probability Value between 0 and 1. Probability of this dependency to get executed. If
     *        not executed, also descendants are not executed.
     * @param alternativeProbability Value between 0 and 1. Probability of this dependency to get
     *        executed. when being a direct child of an {@link AlternativeDependencyDescription}.
     */
    ...
    private DependencyDescription selectOneChildForExecution(final Random random) {
        ...
        /*
         * Sum up the individual (weighted) probabilities until reaching the random number.
         */
        ...
        for (DependencyDescription child : this.getChildren()) {
            cumulatedWeightedSelectionProbability +=
                    child.getAlternativeProbability() / totalWeight;
     ...
     * 
     * @return usually this value is 1.0. However, if not, then the alternative probabilities should
    ...
    /**
     * Thrown when the children of an alternative dependency do not have proper weights, making the
     * decision of picking one dependency impossible.
     */
